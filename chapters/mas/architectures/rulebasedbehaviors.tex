...
\begin{frame}{Unification}
	...
	\vspace{.25cm}
	\begin{definitionblock}{Free variable}
		Each wildcard corresponds to the name a free variable
	\end{definitionblock}
	\begin{definitionblock}{Unification}
...
\begin{frame}{Methods for Rule Application}
...
\begin{frame}{Rule Application with Naive Algorithm}
	\begin{block}{Inputs}
		\begin{itemize}
...
\begin{frame}[t,fragile]{{Pseudo-Code:} Data Matcher}
	\vspace{-.25cm}
	\begin{sarllisting}[basicstyle=\scriptsize]
/* The atomic part of a IF-clause expression is represented by this class
 */
...
\begin{frame}[t,fragile]{{Pseudo-Code:} Group Matcher}
	\vspace{-.25cm}
	\begin{sarllisting}[basicstyle=\scriptsize]
/* A data group test will match a database data group if its identifier matches
 * and if all its children match at least one child of the database group
 * Example: (?anyone (Health 0-54)) on (Captain (Health 43) (Ammo 140))
 *          should match
 */
...
\begin{frame}[t,fragile]{{Pseudo-Code:} Rule System}
	\begin{sarllisting}[basicstyle=\scriptsize]
...
\begin{frame}{{Rule Arbitration:} Most Specific Condition}
	\begin{definitionblock}{Rule Specialization}
	\begin{itemize}
