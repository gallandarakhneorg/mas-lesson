\sidecite{Orkin2003,Riemsdijk08}
...
\begin{frame}[t]{{Action Selection:} Most-Insistense Algorithm}
	\vspace{1cm}
	...
	\begin{example}[at time \only<1-3>{$t$}\only<4>{$t+1$}]
		\begin{tabularx}{\linewidth}{lXXp{1cm}}
...
\begin{frame}[t,fragile]{{Discontentment Pseudo-Code:} Discontentment}
	\begin{sarllisting}[basicstyle=\scriptsize]
...
\begin{frame}[t]{{Include Time} in Goal-Oriented Decision}
	\vspace{1cm}
...
\begin{frame}{{Timed GOB:} Example}
	\begin{example}[at time \only<-3>{$t$}\only<4>{$t+1$}]
		\begin{tabularx}{\linewidth}{lXXp{2cm}}
