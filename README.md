...
* The slides are compiled with **LaTeX Beamer**.
* The slides are based on the [CIAD Beamer template](https://www.arakhne.org/latex/tex-templates/).
* The slides uses LaTeX packages from [AutoLaTeX](https://www.arakhne.org/latex/autolatex/).

### Automatic Installation of requirements

The script `install_tex_libs.py` automatically **download and copy** the needed files from the CIAD Beamer template and AutoLaTeX.

Just run the script:

```bash
$> python3 install_tex_libs.py
```

### Compilation Tools

It is recommended to use [AutoLaTeX](http://www.arakhne.org/autolatex) for compilation:
...
**Alexandre Lombard** <alexandre.lombard@utbm.fr>
**Nicolas Gaud** <nicolas.gaud@utbm.fr>
